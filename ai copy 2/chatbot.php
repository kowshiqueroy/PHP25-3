...
function analyzeNLU($userInput, $conn) {
    ...
    $lastSubject = getMemory('_last_subject', $userId, $conn);
    ...
    if ($intent === 'unknown') {
        ...
        $cleanedSearchQuery = trim($cleanedSearchQuery);

        // Contextual search: if last subject exists and current query is ambiguous
        $ambiguousPhrases = ['tell me more', 'where did he live', 'what is his job', 'about him', 'about her', 'about it', 'more'];
        $isAmbiguous = false;
        foreach ($ambiguousPhrases as $phrase) {
            if (strpos($userInputLower, $phrase) !== false) {
                $isAmbiguous = true;
                break;
            }
        }

        if ($lastSubject && $isAmbiguous) {
            // Combine last subject with current query
            if (strpos($userInputLower, 'tell me more') !== false || strpos($userInputLower, 'more') !== false) {
                $entities['query'] = $lastSubject; // Just search for the subject again
            } else {
                $entities['query'] = $lastSubject . " " . $cleanedSearchQuery;
            }
        } else {
            $entities['query'] = $cleanedSearchQuery;
        }
...
function getResponse($userInput, $conn) {
    ...
    $waitingForDataQuery = getMemory('_waiting_for_data_for_query', $userId, $conn);
    $expectingDataForQuery = getMemory('_expecting_data_for_query', $userId, $conn);

    // Handle multi-turn data input
    if ($waitingForDataQuery) {
        if ($intent === 'affirmative') { // Use intent here
            setMemory('_expecting_data_for_query', $waitingForDataQuery, $userId, $conn);
            setMemory('_waiting_for_data_for_query', '', $userId, $conn); // Clear the flag
            return "Okay, what is the data related to \"" . $waitingForDataQuery . "\"?";
        } elseif ($intent === 'negative') { // Use intent here
            setMemory('_waiting_for_data_for_query', '', $userId, $conn); // Clear the flag
            return "Okay, I won't remember that for now.";
        } else {
            // If user responds with something else while waiting for yes/no
            setMemory('_waiting_for_data_for_query', '', $userId, $conn); // Clear the flag
            return "I was expecting a 'yes' or 'no'. Let's move on.";
        }
    }

    if ($expectingDataForQuery) {
        // User is providing the data
        setMemory($expectingDataForQuery, $userInput, $userId, $conn);
        setMemory('_expecting_data_for_query', '', $userId, $conn); // Clear the flag
        return "Got it! I've saved \"" . $userInput . "\" for \"" . $expectingDataForQuery . "\".";
    }
    ...
    switch ($intent) {
        ...
        case 'search_web':
            ...
            // If web search was successful, store the query as the last subject
            if (strpos($response, "Sorry, I'm not getting any data") === false && strpos($response, "Sorry, I cannot perform web searches") === false) {
                setMemory('_last_subject', $entities['query'], $userId, $conn);
            } else {
                // If web search failed, clear the last subject and set a flag for data input
                setMemory('_last_subject', '', $userId, $conn);
                setMemory('_waiting_for_data_for_query', $entities['query'], $userId, $conn);
            }
            return $response;
        case 'story':
